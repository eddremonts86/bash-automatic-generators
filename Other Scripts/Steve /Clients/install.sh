...
#_site_confg=UserManagement.dk.sh

_urlconf=$(pwd)
#. $_urlconf/sh_config/$_site_confg --source-only
/home/edd/All-Projects/Git Servers/Rebel Penguin Server/steve-intalation/sh_config/$_site_confg --source-only
...
#synfony extentions
echo "---------------------------------"
echo "Composer extentions - Dependencies"
echo "---------------------------------"
...
echo "---------------------------------"
echo "Change permissions - only in local dev-env"
echo "---------------------------------"
    cd ../
...
echo " We are done !!! Enjoy your site. "
exit
